...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:DARK BOT\n'
            + 'ORG:Owner Caanzz;\n'
            + 'TEL;type=CELL;type=VOICE;waid=62813276942007 :+62 813-2769-42007*
            + 'END:VCARD'

prefix = "#"
name = "~ MY BOT"
rdaftar = "HALLO KAK TERIMAKASIH UDH DAFTAR YAA😁"
rmenu = "HAI KAK!😊"
botinfo = "MAU SEWA DONASI DLU YA KAK:)"
limitt = 9999999999
memberLimit = 2
ban = ["6283806369677@s.whatsapp.net"]
premium = ["62813276942007 @s.whatsapp.net"]
...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["0813276942007 @s.whatsapp.net"] // ubah aja gapapa
			...
			mess = {
				...
				only: {
					...
					ownerB: '*𝗠𝗔𝗔𝗙 𝗔𝗡𝗗𝗔 𝗧𝗜𝗗𝗔𝗞 *CANZ* 𝗨𝗡𝗧𝗨𝗞 𝗠𝗘𝗡𝗚𝗚𝗨𝗡𝗔𝗞𝗔𝗡 𝗙𝗜𝗧𝗨𝗥 𝗜𝗡𝗜/𝗞𝗛𝗨𝗦𝗨𝗦 𝗢𝗪𝗡𝗘𝗥* ',
					premium: '*𝗙𝗜𝗧𝗨𝗥 𝗧𝗘𝗥𝗞𝗨𝗡𝗖𝗜!!! 𝗞𝗛𝗨𝗦𝗨𝗦 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗦𝗜𝗟𝗔𝗛𝗞𝗔𝗡 𝗖𝗛𝗔𝗧 𝗢𝗪𝗡𝗘𝗥 𝗚𝗥𝗔𝗧𝗜𝗦!!!*',
					userB: `*Hallo Kak* ${pushname2} *Kayanya Kamu Blm Terdaftar Sebagai Teman MY BOT*\n*Ketik ${prefix}daftar nama|umur*\n*Contoh ${prefix}daftar anker|17*`,
			...
			const ownerNumber = ["62813276942007 @s.whatsapp.net"] // owner number ubah aja
		...
		case prefix+'daftar':
			...
			//if (!isPublic) return reply(mess.only.publikG)
				...
				case prefix+'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nama Bot* : ${me.name}\n➽ *Owner Bot* : wa.me/62813276942007 \n➽ *prefix* : | ${prefix} |\n➽ *Total Block* : ${blocked.length}\n➽ *Aktif Sejak* : ${kyun(uptime)}\n\n➽ *Total Pengguna* : ${user.length} User\n➽ *Instagram* : https://www.instagram.com/Keishaail\n➽ *Special Thanks To* :\n\n➽ Tuhan \n➽ BAGOES FF\n➽ MR. 108P\n➽ Riu\n➽ANKER PRODUCTION \n➽https://github.com/4NK3R-PRODUCT1ON /bot-wea-v2`
         ...
         case prefix+'lirik':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('62813276942007@s.whatsapp.net', options, text, {quoted: mek})
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('62813276942007@s.whatsapp.', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case prefix+'map':
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 MY BOT 〙', members_id, true)
					break
			    case prefix+'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *MY BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				case prefix+'kicktime':
					...
					setTimeout( () => {
					dms.sendMessage(from, `mwehh Kick @${mentioned[0].split('@')[0]}`, text) // ur cods
					}, 5000)
					setTimeout( () => {
